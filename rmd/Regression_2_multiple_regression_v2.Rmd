...
# get the output format for stargazer
# rsults = `asis`
...
# RSE <- sqrt(MSE)
# confint(fit3)
# 2*pnorm(2.84, lower.tail = FALSE)n # pvalues
```
...
# confint(fit3)
# 2*pnorm(2.84, lower.tail = FALSE)n # pvalues for Length
...
```{r results=TRUE}
Anova(fit.no.interation)    ### anova(fit.no.interation) # doesn't give us the right F test!!!!
```
...
summary(fit.all)
# data3 <- data2; data3$Cylinders <- as.factor(data3$Cylinders)
...
Summary about linear models:
   ...
   7. Last but not least: linear models used here only show associations between predictors and the response on average. We CANNOT conclude causality at all.
