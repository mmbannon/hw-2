...
pacman::p_load(ISLR, tidyverse, data.table, skimr, ggplot2, dplyr) # add the packages needed
```
...
@Chris @Charles
Observations: (1) negative income (2) Blank values for Job description 
TBC: have not finished, but got bored
...
SE_87_ggplot <- ggplot(data.esteem.87.short, aes(x=Esteem, y=Score)) 
SE_87_ggplot + geom_histogram() 
...
4. PCA on 10 esteem measurements. (centered but no scaling)
    ...
    a) Report the PC1 and PC2 loadings. Are they unit vectors? Are they uncorrelated? @Charles @Chris - aren't they not unit vectors because we didn't scale it? Idk how to show this... 
    
See in below output for PC1 and PC2 loadings. No, they are not unit vectors because they are not scaled. No, the loadings for PC1 and PC2 ar enot uncorrelated. 
...
Using the elbow method, the optimal number of clusters is 3 (k=3). 
...
SE_data_lm <- data.frame(
  ...
  PC1 = PC1,
  ln_Income05 = SE_data$ln_Income05, 
  ln_Income87 = SE_data$ln_Income87, 
  job_type = SE_data$Job05, 
  BMI = SE_data$BMI, 
...
model_1 <- lm(PC1 ~ . -  BMI - Imag - Ilib - ln_Income87, data=SE_data_lm)
summary(model_1)
